package net.foulest.fstaff.utils.command;
        ...
        for (Method method : obj.getClass().getMethods()) {
            if (method.getAnnotation(Command.class) != null) {
                ...
                if (method.getParameterTypes().length > 1 || method.getParameterTypes()[0] != CommandArgs.class) {
                    MessageUtil.messagePlayer(Bukkit.getConsoleSender(), "&cUnable to register command "
                ...
                if (method.getParameterTypes().length != 1 || method.getParameterTypes()[0] != CommandArgs.class) {
                    MessageUtil.messagePlayer(Bukkit.getConsoleSender(), "&cUnable to register tab completer "
                            + method.getName() + ". Unexpected method arguments");
                    continue;
                }
        ...
        if (map.getCommand(cmdLabel) instanceof BukkitCommand) {
            ...
            try {
                ...
                if (field.get(command) == null) {
                    ...
                    MessageUtil.messagePlayer(Bukkit.getConsoleSender(), "&cUnable to register tab completer "
                            + m.getName() + ". A tab completer is already registered for that command!");
                }
